...
public class BasicSlider : MonoBehaviour
{
    ...
    private void Start()
    {
        Debug.Log(FindObjectsOfType<RefImageChanger>().Length);
        ...
        {
            sliderLength += contentToSlide[i].refTransform.sizeDelta;
    ...
    public void Slide()
    {
        for (int i = 0; i < contentToSlide.Length; i++)
        {
            Tweener tween = contentToSlide[i].refTransform.DOAnchorPos(new Vector2(contentToSlide[i].refTransform.anchoredPosition.x + contentToSlide[i].refTransform.sizeDelta.x * direction.x, contentToSlide[i].refTransform.anchoredPosition.y + contentToSlide[i].refTransform.sizeDelta.y * direction.y), transitionTime).SetEase(ease);
